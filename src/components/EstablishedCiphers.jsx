...
function EstablishedCiphers() {
    ...
    const handleEncrypt = async () => {
        ...
        try {
            ...
            if (outputFormat === 'hex') setResultText(bytesToHex(encryptedBytes));
                 ...
                 else { setResultText(bytesToString(encryptedBytes)); }
             }
    ...
    const handleDecrypt = async () => {
         ...
         try {
             ...
             else { console.warn("Decrypting with 'text' format."); inputBytes = stringToBytes(inputText); }
             let decryptedBytes;
  ...
  return (
    <div className="established-container">
      <h2>Encrypt & Decrypt Established Ciphers</h2>
        ...
        <div className="form-group"> <label htmlFor="cipher-text">Plaintext (for Encrypt) / Ciphertext (for Decrypt):</label> <textarea id="cipher-text" rows="5" placeholder="Enter text. For decrypt, paste ciphertext in the selected format." value={inputText} onChange={(e) => setInputText(e.target.value)} disabled={isProcessing}/> </div>
        <div className="form-group"> <label htmlFor="cipher-key">Key / Password:</label> <input id="cipher-key" type="password" placeholder="Enter secret key or password" value={key} onChange={(e) => setKey(e.target.value)} disabled={isProcessing}/> <small>AES uses PBKDF2 key derivation. OTP uses the key directly.</small> </div>
        <div className="form-group"> <label>Ciphertext Format (Encrypt Output / Decrypt Input):</label> <div className="option-row"> <label><input type="radio" name="outputFormat" value="hex" checked={outputFormat === 'hex'} onChange={() => setOutputFormat('hex')} disabled={isProcessing}/> Hex</label> <label><input type="radio" name="outputFormat" value="base64" checked={outputFormat === 'base64'} onChange={() => setOutputFormat('base64')} disabled={isProcessing}/> Base64</label> </div> </div>
        <div className="button-group"> <button onClick={handleEncrypt} disabled={isProcessing}>{isProcessing ? 'Processing...' : 'Encrypt'}</button> <button onClick={handleDecrypt} className="secondary" disabled={isProcessing}>{isProcessing ? 'Processing...' : 'Decrypt'}</button> </div>
      ...
      <div className="educational-text">
       ...
       <div className="educational-text">
         ...
         <p>
           <strong>Key Derivation (PBKDF2):</strong> Since AES requires a fixed-size binary key, but users typically provide passwords, we use a Key Derivation Function (PBKDF2) here. PBKDF2 takes the password, and applies a hashing function many times to produce a cryptographically strong key of the required length.
         </p>
          <p>
           <strong>IV (Initialization Vector):</strong> GCM requires a unique IV for each encryption performed with the same key. Using the same IV twice with the same key compromises security catastrophically. This implementation generates a random 12-byte IV for each encryption and prepends it to the ciphertext.
         </p>
       </div>
    </div>
  );
